...
@Component({
  ...
  template: `
    <ejs-dialog #smsDialog [header]="'Send SMS Message'" [visible]="false"
                [showCloseIcon]="true" [target]="'body'"
                [width]="'520px'" [isModal]="true" [animationSettings]="{ effect: 'Zoom' }"
                cssClass="comm-dialog">
      <ng-template #content>
        <div class="comm-form-container p-6">
          <div class="context-card sms-card mb-6">
            <div class="card-section border-r border-blue-100/50">
              <div class="label">Regarding</div>
              <div class="value font-black text-blue-900">Claim #{{ claimId }}</div>
            ...
            <div class="card-icon absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white rounded-full p-2 shadow-sm border border-blue-50">
          ...
          <div class="form-group mb-6">
            <div class="flex items-center justify-between mb-2">
              ...
              <div class="text-[10px] uppercase font-black text-blue-500 tracking-widest opacity-60">{{ messageBody.length }} chars</div>
            </div>
            <div class="message-input-wrapper rounded-2xl p-1 bg-gray-50 border border-gray-100 focus-within:border-blue-500/50 focus-within:ring-4 focus-within:ring-blue-500/5 transition-all">
          ...
          <div class="form-group mb-4">
            ...
            <div class="uploader-wrapper p-4 border-2 border-dashed border-gray-200 rounded-xl hover:border-blue-400 transition-colors">
          ...
          <div class="mt-4">
            <ejs-checkbox label="Create follow-up reminder" [(ngModel)]="createFollowUp" cssClass="modern-checkbox"></ejs-checkbox>
          </div>
        </div>
      </ng-template>
  ...
  styles: [`
    ...
    .sms-card { background: linear-gradient(to right, #eff6ff, #f8fafc); border-color: #dbeafe; }
    .card-section {
      flex: 1;
      padding: 16px 20px;
      display: flex;
...
export class SmsDialogComponent implements OnInit {
  @ViewChild('smsDialog') public smsDialog!: DialogComponent;
  ...
  private resetForm(): void {
    this.messageBody = `Hi ${this.party?.firstName || ''}, this is Regarding Claim #${this.claimId}. `;
  ...
  public async send(): Promise<void> {
    ...
    try {
      ...
      this.isSending = false;
      this.toast.success('Sent', 'SMS message has been sent successfully.');
      this.sent.emit();
      this.close();
    } catch (error) {
      this.isSending = false;
      this.toast.error('Error', 'Failed to send SMS message.');
    }
  }
}
